...
// app-pay-admin (same shape as app-task's logger):
...
//   - GinRecovery: panic-recovery middleware writing to slog
...
type FileOptions struct {
	Path       string // log file path (default /app/logs/app-pay-admin.log)
...
func newFileWriter(f FileOptions) io.Writer {
	if f.Path == "" {
		f.Path = "/app/logs/app-pay-admin.log"
	}
	if f.MaxSize == 0 {
		f.MaxSize = 100
	}
