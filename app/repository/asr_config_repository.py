"""
ASRConfigRepository — user_asr_configs 表的 CRUD 操作
"""
...
from app.models import UserASRConfig

_ALIVE = UserASRConfig.deleted_at == None  # noqa: E711
...
class ASRConfigRepository:
    ...
    async def list_by_uid(
        ...
        result = await db.execute(
            select(UserASRConfig)
            .where(UserASRConfig.uid == uid, _ALIVE)
            .order_by(UserASRConfig.updated_at.desc())
        )
        return list(result.scalars().all())

    async def get_by_id(
        self, config_id: int, db: AsyncSession
    ) -> Optional[UserASRConfig]:
        result = await db.execute(
            select(UserASRConfig).where(UserASRConfig.id == config_id)
        )
    ...
    async def create(
        ...
        logger.info(f"[ASR_REPO] created id={record.id} uid={uid}")
        return record
    ...
    async def update(
        ...
        await db.commit()
        await db.refresh(record)
        logger.info(f"[ASR_REPO] updated id={config_id}")
        return record
    ...
    async def delete(self, uid: int, config_id: int, db: AsyncSession) -> bool:
        ...
        await db.commit()
        logger.info(f"[ASR_REPO] soft-deleted id={config_id}")
        return True
    ...
    async def get_default(
        ...
        result = await db.execute(
            select(UserASRConfig).where(
                UserASRConfig.uid == uid,
                UserASRConfig.is_default == True,  # noqa: E712
                _ALIVE,
            )
        )
        return result.scalar_one_or_none()
    ...
    async def _clear_default(
        ...
        stmt = (
            update(UserASRConfig)
            .where(UserASRConfig.uid == uid, UserASRConfig.is_default == True, _ALIVE)  # noqa: E712
            .values(is_default=False)
        )
        if exclude_id is not None:
            stmt = stmt.where(UserASRConfig.id != exclude_id)
...
_asr_repo: Optional[ASRConfigRepository] = None


def get_asr_config_repository() -> ASRConfigRepository:
    global _asr_repo
    if _asr_repo is None:
        _asr_repo = ASRConfigRepository()
    return _asr_repo
