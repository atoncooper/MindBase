...
async def insert_questions(
    ...
    for q in questions:
        ...
        docs.append({
            ...
            "options": json.dumps(_options) if _options is not None and not isinstance(_options, str) else _options,
            "correct_answer": json.dumps(_correct) if not isinstance(_correct, str) else _correct,
            "explanation": q.get("explanation"),
            "keywords": json.dumps(_keywords) if _keywords is not None and not isinstance(_keywords, str) else _keywords,
            "answer_template": q.get("answer_template"),
            "model_answer": q.get("model_answer"),
            "scoring_rubric": json.dumps(_rubric) if _rubric is not None and not isinstance(_rubric, str) else _rubric,
            "bvid": q.get("bvid"),
...
async def get_questions(quiz_uuid: str) -> list[dict]:
    ...
    if not is_enabled():
        logger.warning(f"[MONGO_QUIZ] MongoDB disabled, cannot read questions for {quiz_uuid}")
        return []

    cursor = coll(COLLECTION).find(
        {"quiz_uuid": quiz_uuid, "is_valid": True},
        {
            "question_uuid": 1, "question_type": 1, "difficulty": 1,
            "question_text": 1, "options": 1, "_id": 0,
        },
    )
    results = await cursor.to_list(length=200)
    logger.info(f"[MONGO_QUIZ] get_questions quiz_uuid={quiz_uuid} count={len(results)}")
    return results
...
async def get_questions_full(quiz_uuid: str) -> list[dict]:
    ...
    if not is_enabled():
        logger.warning(f"[MONGO_QUIZ] MongoDB disabled, cannot read full questions for {quiz_uuid}")
        return []
    ...
    results = await cursor.to_list(length=200)
    logger.info(f"[MONGO_QUIZ] get_questions_full quiz_uuid={quiz_uuid} count={len(results)}")
    return results
...
async def delete_by_quiz(quiz_uuid: str) -> int:
    ...
    if not is_enabled():
        return 0
    result = await coll(COLLECTION).delete_many({"quiz_uuid": quiz_uuid})
    logger.info(f"[MONGO_QUIZ] deleted {result.deleted_count} questions for {quiz_uuid}")
