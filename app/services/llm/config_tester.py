...
class ConfigTester:
    ...
    async def test_llm(self, api_key: str, base_url: str, model: str) -> TestResult:
        ...
        try:
            ...
            if not content or len(str(content).strip()) == 0:
                return TestResult(status="error", error="API 返回了空响应", latency_ms=latency)
            logger.info(f"[CONFIG_TESTER] LLM test ok model={model} latency={latency:.0f}ms")
            ...
            msg = str(e)
            if "401" in msg or "403" in msg or "unauthorized" in msg.lower() or "invalid_api_key" in msg.lower():
                hint = "API Key 无效，请检查密钥是否正确"
            elif "timeout" in msg.lower() or "timed out" in msg.lower():
                hint = "连接超时，请检查 API 地址或网络"
            elif "connection" in msg.lower() or "refused" in msg.lower() or "unreachable" in msg.lower():
                hint = f"无法连接到 API 端点: {base_url}"
            elif "not found" in msg.lower() or "404" in msg:
                hint = f"API 端点不存在 (404): {base_url}"
            else:
                hint = msg[:200]
            return TestResult(status="error", error=hint, latency_ms=(time.time() - t0) * 1000)

    # ── Embedding ──────────────────────────────────────────────

    async def test_embedding(self, api_key: str, base_url: str, model: str) -> TestResult:
        ...
        try:
            ...
            if not vectors or len(vectors) == 0:
                return TestResult(status="error", error="API 返回了空向量", latency_ms=latency)
            logger.info(f"[CONFIG_TESTER] Embedding test ok model={model} dims={len(vectors)} latency={latency:.0f}ms")
            return TestResult(status="ok", latency_ms=latency)
    ...
    async def test_asr(self, api_key: str, base_url: str) -> TestResult:
        ...
        try:
            ...
            return TestResult(status="error", error=f"无法连接到 ASR API 端点: {base_url}", latency_ms=(time.time() - t0) * 1000)
        except Exception as e:
            return TestResult(status="error", error=f"连接失败: {str(e)[:200]}", latency_ms=(time.time() - t0) * 1000)
