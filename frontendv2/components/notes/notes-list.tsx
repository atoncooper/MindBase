"use client";

/**
 * NotesList - left sidebar of the notes view.
 *
 * Searchable, grouped (置顶 / 今日 / 昨日 / 本周 / 本月 / 更早) list of note
 * metadata. Pin toggle surfaces on row hover (always-on when pinned). New-note
 * button in the header. Local (client-side) filtering on title + target label.
 *
 * Aesthetic: Apple Notes × Mail sidebar - calm high-legibility list, a colored
 * target-type dot per row for scanning, and a left accent bar marking the
 * active note.
 */
import { useMemo, useState } from "react";
import { motion } from "framer-motion";
import { Search, X, Plus, Pin, NotebookPen } from "lucide-react";
import type { NoteMeta } from "@/lib/api/notes";
import { cn } from "@/lib/utils";

interface NotesListProps {
    notes: NoteMeta[];
    loading: boolean;
    selectedUuid: string | null;
    onSelect: (uuid: string) => void;
    onNew: () => void;
    onTogglePin: (uuid: string, isPinned: boolean) => void;
}

interface Group {
    label: string;
    items: NoteMeta[];
}

// Colored dot per target type - a scannable cue (à la Mail account dots).
function targetDot(note: NoteMeta): string {
    if (note.targetId.startsWith("scratch:")) return "bg-tertiary";
    if (note.targetType === "video") return "bg-accent";
    if (note.targetType === "cloud_file") return "bg-success";
    return "bg-tertiary";
}

function targetLabel(note: NoteMeta): string {
    if (note.targetId.startsWith("scratch:")) return "速记";
    if (note.targetType === "video") return "视频";
    if (note.targetType === "cloud_file") return "云盘";
    return "笔记";
}

function timeAgo(iso: string): string {
    const date = new Date(iso);
    if (Number.isNaN(date.getTime())) return "";
    const diff = Date.now() - date.getTime();
    const min = Math.floor(diff / 60000);
    const hour = Math.floor(diff / 3600000);
    const day = Math.floor(diff / 86400000);
    if (min < 1) return "刚刚";
    if (min < 60) return `${min} 分钟前`;
    if (hour < 24) return `${hour} 小时前`;
    if (day < 7) return `${day} 天前`;
    return `${date.getMonth() + 1}月${date.getDate()}日`;
}

function bucketOf(iso: string): string {
    const date = new Date(iso);
    if (Number.isNaN(date.getTime())) return "更早";
    const now = new Date();
    const startOfToday = new Date(now.getFullYear(), now.getMonth(), now.getDate()).getTime();
    const ts = date.getTime();
    const dayMs = 86400000;
    if (ts >= startOfToday) return "今日";
    if (ts >= startOfToday - dayMs) return "昨日";
    if (ts >= startOfToday - 6 * dayMs) return "本周";
    if (ts >= startOfToday - 29 * dayMs) return "本月";
    return "更早";
}

function groupNotes(notes: NoteMeta[]): Group[] {
    const pinned = notes.filter((n) => n.isPinned);
    const rest = notes.filter((n) => !n.isPinned);
    const order = ["今日", "昨日", "本周", "本月", "更早"];
    const buckets = new Map<string, NoteMeta[]>();
    for (const n of rest) {
        const key = bucketOf(n.updatedAt);
        if (!buckets.has(key)) buckets.set(key, []);
        buckets.get(key)!.push(n);
    }
    const groups: Group[] = [];
    if (pinned.length) groups.push({ label: "置顶", items: pinned });
    for (const key of order) {
        const items = buckets.get(key);
        if (items?.length) groups.push({ label: key, items });
    }
    return groups;
}

export function NotesList({
    notes,
    loading,
    selectedUuid,
    onSelect,
    onNew,
    onTogglePin,
}: NotesListProps) {
    const [query, setQuery] = useState("");

    const filtered = useMemo(() => {
        const q = query.trim().toLowerCase();
        if (!q) return notes;
        return notes.filter(
            (n) =>
                n.title.toLowerCase().includes(q) ||
                targetLabel(n).toLowerCase().includes(q)
        );
    }, [notes, query]);

    const groups = useMemo(() => groupNotes(filtered), [filtered]);

    return (
        <div className="flex h-full flex-col bg-surface">
            {/* Header */}
            <div className="flex items-center justify-between px-4 pb-2.5 pt-4">
                <h2 className="text-[15px] font-semibold tracking-tight text-foreground">笔记</h2>
                <button
                    type="button"
                    onClick={onNew}
                    title="新建笔记"
                    aria-label="新建笔记"
                    className="grid h-7 w-7 place-items-center rounded-full text-secondary transition-all hover:bg-border-subtle hover:text-foreground active:scale-95"
                >
                    <Plus className="h-4 w-4" />
                </button>
            </div>

            {/* Search */}
            <div className="px-3 pb-2.5">
                <div
                    className={cn(
                        "flex h-8 items-center gap-2 rounded-[10px] border border-transparent bg-border-subtle/70 px-2.5 transition-colors",
                        "focus-within:bg-surface focus-within:border-accent/40 focus-within:shadow-[0_0_0_3px_rgba(0,113,227,0.12)]"
                    )}
                >
                    <Search className="h-3.5 w-3.5 shrink-0 text-tertiary" />
                    <input
                        value={query}
                        onChange={(e) => setQuery(e.target.value)}
                        placeholder="搜索…"
                        className="min-w-0 flex-1 bg-transparent text-[12px] text-foreground outline-none placeholder:text-tertiary"
                    />
                    {query && (
                        <button
                            type="button"
                            onClick={() => setQuery("")}
                            aria-label="清除搜索"
                            className="grid h-4 w-4 shrink-0 place-items-center rounded-full text-tertiary transition-colors hover:bg-border-subtle hover:text-foreground"
                        >
                            <X className="h-3 w-3" />
                        </button>
                    )}
                </div>
            </div>

            {/* List */}
            <div className="min-h-0 flex-1 overflow-y-auto px-2 pb-2">
                {loading && notes.length === 0 ? (
                    <div className="space-y-0.5 px-1 pt-1">
                        {Array.from({ length: 7 }).map((_, i) => (
                            <div key={i} className="rounded-[10px] px-2.5 py-2">
                                <div className="h-3 w-2/3 animate-pulse rounded bg-border-subtle" />
                                <div className="mt-2 h-2 w-1/3 animate-pulse rounded bg-border-subtle/60" />
                            </div>
                        ))}
                    </div>
                ) : filtered.length === 0 ? (
                    <div className="flex flex-col items-center gap-3 px-6 pt-20 text-center">
                        {query ? (
                            <>
                                <div className="grid h-10 w-10 place-items-center rounded-2xl bg-border-subtle">
                                    <Search className="h-4 w-4 text-tertiary" />
                                </div>
                                <div className="space-y-0.5">
                                    <p className="text-[13px] font-medium text-foreground/80">无匹配笔记</p>
                                    <p className="text-[11px] text-tertiary">没有「{query}」相关的笔记</p>
                                </div>
                            </>
                        ) : (
                            <>
                                <div className="grid h-11 w-11 place-items-center rounded-2xl bg-border-subtle">
                                    <NotebookPen className="h-5 w-5 text-tertiary" />
                                </div>
                                <div className="space-y-0.5">
                                    <p className="text-[13px] font-medium text-foreground/80">还没有笔记</p>
                                    <p className="text-[11px] text-tertiary">落笔写下第一条吧</p>
                                </div>
                                <button
                                    type="button"
                                    onClick={onNew}
                                    className="rounded-full bg-accent-soft px-3 py-1.5 text-[12px] font-medium text-accent transition-colors hover:bg-accent/15"
                                >
                                    新建笔记
                                </button>
                            </>
                        )}
                    </div>
                ) : (
                    groups.map((group) => (
                        <div key={group.label} className="mb-1.5">
                            <div className="flex items-center gap-2 px-2.5 pb-1 pt-3">
                                <span className="text-[10px] font-semibold uppercase tracking-[0.14em] text-tertiary">
                                    {group.label}
                                </span>
                                <span className="text-[10px] tabular-nums text-tertiary/60">
                                    {group.items.length}
                                </span>
                                <div className="h-px flex-1 bg-border-subtle" />
                            </div>
                            {group.items.map((note, i) => {
                                const isActive = note.uuid === selectedUuid;
                                return (
                                    <motion.div
                                        key={note.uuid}
                                        initial={{ opacity: 0, y: 3 }}
                                        animate={{ opacity: 1, y: 0 }}
                                        transition={{
                                            duration: 0.3,
                                            delay: Math.min(i * 0.018, 0.12),
                                            ease: [0.28, 0.11, 0.32, 1],
                                        }}
                                    >
                                        <button
                                            type="button"
                                            onClick={() => onSelect(note.uuid)}
                                            aria-current={isActive ? "true" : undefined}
                                            className={cn(
                                                "group relative w-full rounded-[10px] px-2.5 py-2 text-left transition-colors duration-150",
                                                isActive ? "bg-accent-soft" : "hover:bg-border-subtle/70"
                                            )}
                                        >
                                            {isActive && (
                                                <span className="absolute bottom-2 left-0 top-2 w-[2.5px] rounded-full bg-accent" />
                                            )}
                                            <div className="flex items-center gap-1.5">
                                                <p
                                                    className={cn(
                                                        "min-w-0 flex-1 truncate text-[13px] leading-snug",
                                                        isActive
                                                            ? "font-medium text-foreground"
                                                            : "text-foreground/90"
                                                    )}
                                                >
                                                    {note.title || "无标题"}
                                                </p>
                                                <span
                                                    role="button"
                                                    tabIndex={0}
                                                    onClick={(e) => {
                                                        e.stopPropagation();
                                                        onTogglePin(note.uuid, note.isPinned);
                                                    }}
                                                    onKeyDown={(e) => {
                                                        if (e.key === "Enter" || e.key === " ") {
                                                            e.preventDefault();
                                                            e.stopPropagation();
                                                            onTogglePin(note.uuid, note.isPinned);
                                                        }
                                                    }}
                                                    className={cn(
                                                        "grid h-5 w-5 shrink-0 place-items-center rounded-md transition-all",
                                                        note.isPinned
                                                            ? "text-accent opacity-100"
                                                            : "text-tertiary opacity-0 hover:text-foreground group-hover:opacity-100"
                                                    )}
                                                    title={note.isPinned ? "取消置顶" : "置顶"}
                                                >
                                                    <Pin
                                                        className={cn("h-3 w-3", note.isPinned && "fill-accent")}
                                                    />
                                                </span>
                                            </div>
                                            <div className="mt-1 flex items-center gap-1.5">
                                                <span
                                                    className={cn(
                                                        "h-1.5 w-1.5 shrink-0 rounded-full",
                                                        targetDot(note)
                                                    )}
                                                />
                                                <span className="min-w-0 truncate text-[11px] text-secondary">
                                                    {targetLabel(note)}
                                                </span>
                                                <span className="text-tertiary/60">·</span>
                                                <span className="shrink-0 text-[11px] text-tertiary">
                                                    {timeAgo(note.updatedAt)}
                                                </span>
                                            </div>
                                        </button>
                                    </motion.div>
                                );
                            })}
                        </div>
                    ))
                )}
            </div>

            {/* Footer - grounds the sidebar with a stable total count. */}
            {!loading && notes.length > 0 && (
                <div className="border-t border-border-subtle px-4 py-2">
                    <span className="text-[10px] tabular-nums text-tertiary/70">
                        共 {notes.length} 条笔记
                    </span>
                </div>
            )}
        </div>
    );
}
