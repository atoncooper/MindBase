 ...
 * webview 外壳；URL 仍保留在 mdSource 里可编辑。`pre` 在 foreignObject
 * 下不可靠（codeBlock 第二十四轮教训），渲染为 div.smm-md-pre。
 */
...
/** 画布/预览共用的 URL 白名单（http(s)/file/data:image/盘符/相对路径）。 */
...
export function renderMdCardHtml(source: string): string {
  const inner = renderToStaticMarkup(
    <Markdown remarkPlugins={MD_PLUGINS} urlTransform={allowCanvasUrl} components={MD_COMPONENTS}>
      {source}
    </Markdown>,
  );
...
export { MD_PLUGINS, MD_COMPONENTS, allowCanvasUrl as MD_URL_TRANSFORM };
